...
const ExpertConfidenceMeter = ({ score }) => {
  ...
  const getScoreLabel = () => {
    if (score >= 90) return "analysis.confidence.expert";
    if (score >= 80) return "analysis.confidence.high";
    if (score >= 70) return "analysis.confidence.medium";
    return "analysis.confidence.caution";
  };
  ...
  return (
    <div className="text-center">
        <div className="text-sm text-gray-400 mb-2">{t('analysis.expertConfidence')}</div>
    ...
    >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">{t('analysis.expertAnalysis')}</h2>
                ...
                <div className="flex items-center justify-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-300">
                      {analysis.entry_action || t('analysis.marketOrder')}
                    </div>
                  </div>
                </div>
                <div className="text-center">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">{t('analysis.type')}:</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">{t('analysis.strategy')}:</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">{t('analysis.timeframe')}:</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">{t('analysis.riskReward')}:</span>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                <h4 className="font-bold text-blue-300 mb-2 text-lg">
                  {analysis.pattern_detected || t('analysis.advancedPattern')}
                </h4>
                <p className="text-gray-300 text-sm mb-3">
                  {analysis.pattern_description || t('analysis.patternDescription')}
                </p>
                <div className="flex items-center gap-2">
                  {analysis.volume_confirmation && (
                    <Badge variant="outline" className="bg-green-500/20 text-green-400 border-green-500/30">
                      {t('analysis.volumeConfirmed')}
                    </Badge>
                  )}
                  <Badge variant="outline" className={riskColors[analysis.risk_level]}>
                    {analysis.risk_level?.toUpperCase()} {t('analysis.risk')}
                  </Badge>
                </div>
              </div>
          ...
          <CardContent>
            <div className="space-y-3">
              <div className={`p-3 rounded-lg bg-${analysis.market_condition === 'bullish' ? 'green' : analysis.market_condition === 'bearish' ? 'red' : 'yellow'}-500/10 border border-${analysis.market_condition === 'bullish' ? 'green' : analysis.market_condition === 'bearish' ? 'red' : 'yellow'}-500/20`}>
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">{t('analysis.marketCondition')}:</span>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
              <div className={`p-4 rounded-lg ${actionBg} border ${actionBorder} text-center`}>
                ...
                <h4 className="font-semibold text-white mb-1">{t('analysis.entry')}</h4>
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
              <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20 text-center">
                ...
                <h4 className="font-semibold text-white mb-1">{t('analysis.stopLoss')}</h4>
                <p className="text-xl font-bold text-red-400">
                  ${analysis.stop_loss?.toFixed(4) || '-'}
                </p>
                <p className="text-xs text-gray-400">{t('analysis.protectCapital')}</p>
              </div>
            </motion.div>
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
              <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20 text-center">
                ...
                <h4 className="font-semibold text-white mb-1">{t('analysis.tp1')}</h4>
                <p className="text-xl font-bold text-green-400">
                  ${analysis.take_profit_1?.toFixed(4) || '-'}
                </p>
                <p className="text-xs text-gray-400">
                  {analysis.estimated_time_tp1 || t('analysis.estimatedTime15to30')}
                </p>
              </div>
            </motion.div>
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
              <div className="p-4 rounded-lg bg-green-600/10 border border-green-600/20 text-center">
                ...
                <h4 className="font-semibold text-white mb-1">{t('analysis.tp2')}</h4>
                <p className="text-xl font-bold text-green-500">
                  ${analysis.take_profit_2?.toFixed(4) || '-'}
                </p>
                <p className="text-xs text-gray-400">
                  {analysis.estimated_time_tp2 || t('analysis.estimatedTime1to2')}
                </p>
              </div>
            </motion.div>
          </div>
            ...
            <div className="flex justify-center mb-6">
              <motion.div whileHover={{ scale: 1.02 }} className="w-full max-w-xs">
                <div className="p-4 rounded-lg bg-green-700/10 border border-green-700/20 text-center">
                  ...
                  <h4 className="font-semibold text-white mb-1">{t('analysis.tp3Bonus')}</h4>
                  <p className="text-xl font-bold text-green-600">
                    ${analysis.take_profit_3?.toFixed(4)}
                  </p>
                  <p className="text-xs text-gray-400">
                    {analysis.estimated_time_tp3 || t('analysis.estimatedTime4plus')}
                  </p>
                </div>
              </motion.div>
            </div>
          ...
          <div className="p-4 rounded-lg bg-purple-500/10 border border-purple-500/20 mb-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-purple-300 mb-2">{t('analysis.expertReasoning')}</h4>
                <p className="text-gray-300 text-sm leading-relaxed mb-3 whitespace-pre-wrap">
                  {analysis.expert_reasoning || analysis.analysis_notes || 
                    t('analysis.defaultReasoning', {
                      direction: analysis.trading_direction,
                ...
                <div className="text-xs text-gray-400">
                  <strong>{t('analysis.note')}:</strong> {t('analysis.positionSizeNote')}
                </div>
              </div>
            </div>
          </div>
          ...
          {analysis.potential_risks && (
            <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-500/20">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-amber-300 mb-2">{t('analysis.potentialRisks')}</h4>
      ...
      <Card className="glass-effect border-orange-500/30 bg-orange-500/5">
        <CardContent className="p-6 flex items-start gap-4">
          ...
          <div>
            <h4 className="font-bold text-orange-400 mb-2">⚠️ {t('analysis.disclaimer.title')}</h4>
            <p className="text-sm text-gray-300 mb-3">
              {t('analysis.disclaimer.description')}
            </p>
            <div className="grid md:grid-cols-2 gap-4 text-xs text-gray-400">
              <div>
                <strong className="text-orange-300">{t('analysis.disclaimer.riskManagement')}:</strong>
                <ul className="ml-4 mt-1 space-y-1">
                  <li>• {t('analysis.disclaimer.rule1')}</li>
                  <li>• {t('analysis.disclaimer.rule2')}</li>
                  <li>• {t('analysis.disclaimer.rule3')}</li>
                </ul>
              </div>
              <div>
                <strong className="text-orange-300">{t('analysis.disclaimer.tradingRules')}:</strong>
                <ul className="ml-4 mt-1 space-y-1">
                  <li>• {t('analysis.disclaimer.rule4')}</li>
                  <li>• {t('analysis.disclaimer.rule5')}</li>
                  <li>• {t('analysis.disclaimer.rule6')}</li>
                </ul>
              </div>
            </div>
          </div>
