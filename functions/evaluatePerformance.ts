...
Deno.serve(async (req) => {
  try {
    ...
    const maxSignals = Math.min(body.maxSignals || 5, 10);
    ...
    if (recentSignals.length === 0) {
      return Response.json({ evaluated: 0, errors: 0, created: [] });
    }
    ...
    const signals = Object.values(deduped).slice(0, maxSignals);
    const results = { evaluated: 0, errors: 0, created: [] };
    ...
    for (const signal of signals) {
      try {
        ...
        if (existing.length > 0) throw new Error('Already evaluated');
    ...
    return Response.json({ error: error.message, evaluated: 0, errors: 1, created: [] }, { status: 500 });
  }
});