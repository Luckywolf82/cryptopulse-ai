...
Deno.serve(async (req) => {
  try {
    ...
    if (signals.length === 0) {
      return Response.json({ message: 'No signals to evaluate', count: 0 });
    }
    ...
    const lookbackMap = {
      '15m': { cryptocompare: 288, holdingCandles: Math.ceil(holdingPeriodHours * 4) }, // 2 days
      '1h': { cryptocompare: 168, holdingCandles: Math.ceil(holdingPeriodHours) }, // 1 week
      '4h': { cryptocompare: 168, holdingCandles: Math.ceil(holdingPeriodHours / 4) }, // 1 week
      '1d': { cryptocompare: 365, holdingCandles: Math.ceil(holdingPeriodHours / 24) } // 1 year
    };
    ...
    for (const signal of uniqueSignals) {
      const promise = (async () => {
        try {
          ...
          results.evaluated++;
          results.created.push({ symbol: signal.symbol, status });
      ...
      if (activePromises.length >= maxConcurrency) {
        ...
        activePromises = activePromises.filter(p => p.status !== 'fulfilled');
      }
    }
