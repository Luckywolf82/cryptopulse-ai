...
Deno.serve(async (req) => {
  try {
    ...
    const maxSignals = body.maxSignals || 50;
    ...
    if (recentSignals.length === 0) {
      return Response.json({ evaluated: 0, errors: 0, created: [], message: 'No signals to evaluate' });
    }
    ...
    const uniqueSignals = Object.values(deduped).slice(0, maxSignals);
    ...
    for (const signal of uniqueSignals) {
      try {
        ...
        if (signalCandleIndex < 0 || signalCandleIndex >= ohlcData.close.length - 1) {
          throw new Error('Signal candle not found');
        }
